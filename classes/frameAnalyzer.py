...
def find_lanes(frame, flip, video):
	...
	# search for parallel lines
	# parallel = parralel(frame, lines)
	# res =  parallel.find_parralel_lines()
	# cv2.line(frame, res[0], res[1], (51, 51, 51), 2)
	# detected = 0
	...
	#
	# 			dy2 = secondLine[0][3] - secondLine[0][1];
	# 			dx2 = secondLine[0][2] - secondLine[0][0];
	# 			avgX2 = (secondLine[0][0] + secondLine[0][2]) / 2
	# 			avgY2 = (secondLine[0][1] + secondLine[0][3]) / 2
	# 			angle2 = int(math.atan2(dy2, dx2) * 180.0 / math.pi);
	# 			print str(angle2) + '***' + str(angle)
	# 			print str(angle2 - 3) + '<' +  str(angle2) +'<'+ str(angle2+3)
	# 			if ( angle2 - 3 < angle < angle2 + 3 ):
	# 				print str((avgX, avgY)) + "====" + str((avgX2, avgY2))
	# 				cv2.circle(frame, (avgX, avgY), 5, get_color_by_angle(angle), -1)
	# 				cv2.circle(frame, (avgX2, avgY2), 5, get_color_by_angle(angle), -1)
	# 				detected +=0
	# 		print '  '
	...
	if (lines is not None) :
		for line in lines:
			...
			groupedLines[get_group_id_by_angle(angle)].append([(line[0][0], line[0][1]), (line[0][2], line[0][3])])
